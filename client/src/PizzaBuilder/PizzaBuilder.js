...
export default function PizzaBuilder() {
    ...
    const setDough = function(type, exclusive){
            ...
            if(exclusive){
                let foundIndex2 = DoughsCopy.findIndex(dough => dough.selected === true);
    ...
    switch (selection) {
        case 'Dough':
            output = <OptionList Dough = {options.Dough} setDough = {setDough('Dough', 'exclusive')} />
            break;
        case 'Crust':
            output = <OptionList Dough = {options.Crust} setDough = {setDough('Crust', 'exclusive')} />
            break;
        case 'Cheese':
            output = <OptionList Dough = {options.Cheese} setDough = {setDough('Cheese', 'exclusive')} />
            break;
        case 'Veggies':
            output = <OptionList Dough = {options.Veggies} setDough = {setDough('Veggies')} />
            break;
        case 'Meat':
            output = <OptionList Dough = {options.Meat} setDough = {setDough('Meat')} />
            break;
        case 'Drink':
            output = <OptionList Dough = {options.Drink} setDough = {setDough('Drink', 'exclusive')} />
            break;
        default:
